""" SICKSense Agripollinate Image Client Version 0.1 
    Created by:    Josiah Faircloth 
    date:    9/30/2025

    Compatible with Image Server Versions 0.1 & 0.2
    
    Recieves an image file from a connected server using 
    Transmission Control Protocol (TCP), then disconnects.    
"""
...
def main():
    ...
    while True:
        ...
        # Ask if user wants to receive another image
        choice = input("\nWait for another image? (y/n): ").strip().lower()
        if choice != 'y':
            break
