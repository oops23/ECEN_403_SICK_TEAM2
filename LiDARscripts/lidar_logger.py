...
def parse_scan(telegram):
    ...
    parts = telegram.strip().split(' ') # Split by spaces
    if 'DIST1' not in parts: # Invalid telegram
        return None

    try:
        num_index = parts.index('DIST1') + 5 # Number of values is 5 after 'DIST1'
        num_values = int(parts[num_index], 16)  
        distances = [int(v, 16) / 1000.0 for v in parts[num_index + 1:num_index + num_values]] # extract distances in meters
        return distances # List of distances in meters
    except Exception: # Parsing error
        return None

def main():
    timestamp = datetime.now().strftime("%m-%d-%Y_%H.%M.%S")
    out_file = f"lidar_log_{timestamp}.jsonl" # Create output file name

    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s: # open TCP socket
        s.connect((HOST, PORT)) # connect to port
        s.sendall(CMD_START) # Send start command
        ...
        with open(out_file, 'w') as f:
            try:
                buffer = ""
                while True:
                    data = s.recv(4096).decode(errors='ignore') # Receive data
                    
                    buffer += data
                    while '\x03' in buffer:  # end of one telegram
                        telegram, buffer = buffer.split('\x03', 1) # Split at end character
                        telegram = telegram.replace('\x02', '').strip() # Remove start character and whitespace
                        distances = parse_scan(telegram) # Parse distances from telegram
                        if distances:
                            # create scan entry
                            scan = { 
                                "timestamp": time.time(),
